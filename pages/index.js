...
const Home = (props) => {
  const { carousels, news, events, bottomcards } = props;
    ...
    <Layout>
      ...
      >
        ...
        <div className="flex justify-center h-20 relative">
          <img src={logo} className="absolute h-20 top-20" alt="123" />
        </div>
      </div>
      {bottomcards?.length && (
        <MainLayout>
...
export async function getStaticProps() {
  ...
  const [carouselsRes, newsRes, eventsRes, homepageRes] = await Promise.all([
    ...
    fetchAPI("/news"),
    fetchAPI("/events"),
    fetchAPI("/bottomcards"),
  ]);

  return {
    props: {
      carousels: carouselsRes.allGallery,
      news: newsRes.allNews,
      events: eventsRes.allEvents,
      bottomcards: homepageRes.allBottomcards,
    },
    revalidate: 1,
  };
}
